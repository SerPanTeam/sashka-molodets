...
const objects={1:"one bright red apple",2:"two bright red apples",3:"three bright red apples",4:"four bright red apples"};
function prompt(extra=""){return `Create a premium square educational number card for a child aged 3 to 7. The card MUST show one large, bold, clearly readable Arabic numeral ${n}, occupying roughly the upper third of the card. It must be the digit ${n}, not a word and not a decorative symbol. Below it, show EXACTLY ${words[n]} (${n}) clearly separated bright red apples. Use apples for EVERY number card so only the quantity changes between cards. The child must instantly connect the written digit ${n} with the exact quantity ${n}. Arrange all ${n} apples in a simple clean row or balanced group with clear space between every apple. Every apple must be fully visible and distinct. NO other fruit, NO people, NO hands, NO fingers, NO animals, NO extra objects, NO partial hidden objects, NO reflections that look like extra objects. No written words, letters, labels, logos or watermarks other than the single Arabic numeral ${n}. Clean very-light warm cream/pastel studio background, polished friendly high-end 3D children's educational illustration, vivid natural colors, soft cinematic studio lighting, square 1:1, generous margins, nothing cropped. ${extra}`;}
...
for(let artAttempt=1;artAttempt<=5;artAttempt++){
  const result=await withRetry(()=>generateImage({provider,prompt:prompt(qaText?`Previous attempt failed QA: ${qaText}. Correct the DIGIT, APPLES, and COUNT above all else.`:""),aspectRatio:"1:1",imageSize:"1K",size:"1024x1024",quality:"premium"}));
  const expected=`An educational number card that visibly contains one large Arabic numeral ${n} and exactly ${n} clearly separated bright red apples below it, with no other fruit, hands or people. The digit must be ${n}, every countable object must be an apple, and there must be neither fewer nor more than ${n} apples.`;
...
if(!accepted){await emit("error",{error:`Failed numeral/apple-count QA: ${qaText}`});process.exit(2)}
