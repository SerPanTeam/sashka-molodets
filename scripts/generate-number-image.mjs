...
function prompt(extra=""){return `Create a premium square educational counting card for a child aged 3 to 7. Show EXACTLY ${words[n]} (${n}) clearly separated countable objects: ${objects[n]}. The child must be able to count them instantly. Arrange all ${n} objects in a simple clean row or balanced group with clear space between every object. Every object must be fully visible and distinct. NO people, NO hands, NO fingers, NO animals, NO extra objects, NO partial hidden objects, NO reflections that look like extra objects. Do not draw any written words, letters, labels, logos or watermarks. Clean very-light warm cream/pastel studio background, polished friendly high-end 3D children's educational illustration, vivid natural colors, soft cinematic studio lighting, square 1:1, generous margins, nothing cropped. The image represents the number ${n} solely by having exactly ${n} countable objects. ${extra}`;}
...
for(let artAttempt=1;artAttempt<=5;artAttempt++){
  const result=await withRetry(()=>generateImage({provider,prompt:prompt(qaText?`Previous attempt failed QA: ${qaText}. Correct the COUNT above all else.`:""),aspectRatio:"1:1",imageSize:"1K",size:"1024x1024",quality:"premium"}));
  const expected=`An educational counting card showing exactly ${n} clearly separated objects and no hands or people. There must be neither fewer nor more than ${n} countable objects.`;
...
if(!accepted){await emit("error",{error:`Failed exact-count QA: ${qaText}`});process.exit(2)}
