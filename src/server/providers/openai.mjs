...
export async function generateImage({
  ...
  return { buffer: Buffer.from(b64, "base64"), mimeType: "image/png", source: "openai" };
}

export async function generateSpeech({
  text,
  voice = process.env.OPENAI_TTS_VOICE || "coral",
  style = "Speak warmly, cheerfully, patiently, and clearly to a young child.",
  ...
  return { buffer: Buffer.from(await response.arrayBuffer()), mimeType: "audio/wav", source: "openai" };
}
